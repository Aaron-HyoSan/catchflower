 ...
 *    **로그인한 뒤에도 그대로 물려받는다.**
 *
 * ## 🔴 2026-08-16: **회원 탈퇴는 일부러 넣지 않았다**
 *
 * 설정 8행 `회원 탈퇴`([SettingsRules.Row.DELETE_ACCOUNT])는 되돌릴 수 없는 행동인데
 * 게이트를 **안 걸었다.** 위 `지역 선택`과 같은 종류의 예외이므로 여기 적는다 —
 * 안 적으면 다음 사람이 "빠뜨린 것"으로 보고 [GatedAction]에 넣는다.
 *
 * 이유: **익명 계정도 서버에 실제로 있는 계정이다**(`handle_new_user()`가 닉네임까지
 * 만든다). 로그인해야 지울 수 있게 하면 **지울 방법이 없는 계정**이 생기고, 그건
 * Play 정책(계정을 만드는 앱은 앱 안에서 삭제할 수 있어야 한다)에도 어긋난다.
 * 게이트를 걸었을 때의 증상은 "탈퇴하려면 먼저 로그인하라는 앱"이고, 사용자가 보기에는
 * **계정을 붙잡는 것**으로 읽힌다.
 *
 * ⚠️ 파괴적 행동을 게이트 밖에 두는 것은 이것 하나뿐이다. 안전은 게이트가 아니라
 *    **확인 다이얼로그**([AccountDeletionRules.Phase])가 맡는다.
 *
 * ## 🔴 왜 화면이 아니라 여기서 판정하는가
 *
