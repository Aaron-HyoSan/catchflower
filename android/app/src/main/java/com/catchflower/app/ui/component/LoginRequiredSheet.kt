...
private fun LoginRequiredSheetBody(
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            // 🔴 **동의 고지다(A 문서 3절 ⑫ · 2026-08-16).** 이용약관 제4조와 개인정보
            //    처리방침 2항이 `만 14세 이상`을 전제로 쓰여 있는데, 그 말을 사용자가
            //    보는 화면이 **한 곳도 없었다** — 그러면 "확인한 것으로 본다"가 성립하지
            //    않는다. 로그인이 이 앱에서 가입에 가장 가까운 행동이라 여기에 둔다.
            //
            // ⚠️ **누를 수 없는 문장이다.** 여기서 문서로 나가면 시트가 닫히고 원래
            //    하려던 행동(판별·댓글·좋아요…)이 사라지는데 돌아올 경로가 없다.
            //    문서는 설정에서 읽는다 — **설정은 로그인 없이 열린다**(확인했다).
            //
            // ⚠️ **한 리터럴로 쓴다.** 두 조각으로 이어 붙였더니 `CopySourceTest`가
            //    앞 조각만 꺼내서 "A 문서에 없는 문구"로 잡혔다 — 그 검사의 정규식은
            //    `Text("…")` 안의 **첫 리터럴**만 본다. 줄이 길어지는 쪽을 고른다.
            Text(
                text = "만 14세 이상만 이용할 수 있어요. 로그인하면 이용약관과 개인정보 처리방침에 동의한 것으로 봅니다.",
