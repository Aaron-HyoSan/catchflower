...
data class Flower(
     ...
     *    내보내 자릿수가 섞인다(`999.webp`와 `1000.webp`). 그래서 자산 이름을 4자리로
     *    통일했고, **이 값과 `SyncSharedAssets`의 정규식·복사 이름은 같이 움직여야
     *    한다.** 한쪽만 고치면 그림만 안 나오고 예외는 없다 — 그 칸이 빌 뿐이다.
     *
     * 🔴 **`.webp`다(2026-08-12).** PNG였다. 전수 2,057장을 PNG로 넣으면 130MB로
     *    Play 업로드 상한(AAB 150MB · APK 직접 100MB)을 넘고,
     *    **빌드는 성공한 뒤 업로드에서 막힌다.**
     *    WebP q80 + alpha_q 100으로 46.4MB가 됐다(잉크영역 오차 3.93/255 · 도감 셀이
     *    쓰는 32px에서 0.09/255 — `pack_illust_webp.py` 주석에 실측표가 있다).
     *    ⚠️ 투명 WebP는 API 18+이고 이 앱은 `minSdk 26`이라 안전하다. 확인 안 하고
     *    갈면 **오래된 기기에서만** 빈 칸이 되고 이 맥의 에뮬레이터로는 안 보인다.
     */
    val illustAssetName: String
        get() = "flower_illust/%04d.webp".format(id)
}
