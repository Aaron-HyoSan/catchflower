...
--
--  ## ✅ 통째로 붙여넣으면 된다 — **1절(미리보기)이 나온다**
--
--  2절(삭제)과 3절(다시 세기)은 **둘 다 `/* */`로 잠겨 있다.** 그래서 이 파일을
--  전체 복사해서 Run 하면 **아무것도 지우지 않고 미리보기 표만** 나온다.
--
--  🔴 **처음에는 3절이 열려 있었고, 그래서 사고가 났다(2026-08-12).**
--     SQL Editor는 여러 문장을 돌리면 **마지막 결과만** 보여준다. 오너가 파일을
--     통째로 붙여넣으니 **3절 표만 뜨고 1절 미리보기가 조용히 가려졌다** —
--     오류도 없고 그럴듯한 표가 하나 떠 있었다. 그 함정을 이 파일 안에 경고로
--     적어 두고도 3절을 열어 놨던 것이 실수다. **문장으로 부탁할 것을 구조로 막았다.**
--
...
--
-- ✅ **2026-08-12 실측: 그대로 나왔다. 17줄 / 7줄.** 검산은 손으로 하지 않았다
--    (`$CLAUDE_JOB_DIR/tmp/verify_preview_table.py`, 13개 통과):
--
--      · 두 조회(분류표 → 미리보기)의 **계정 24개가 같고 기록수도 하나도 안 달라졌다**
--      · 🟢 7개가 🗑로 **한 개도 새지 않았다** ← 이게 진짜 검사다
--      · `9451db26`(42건)은 🟢 · `a31bcf09`는 표에 아예 안 나온다
--      · ⚪️6 + 🔴11 = 17이 전부 🗑 · 기록 19건 · 🟡 줄은 0개
--      · 🗑 기록 19 + 🟢 기록 57 = **76** = 3절의 `남은기록`과 같다
--
--    🔴 **손으로 옮겨 적다가 실제로 한 줄을 틀렸다**(`dd2cb966`을 37.5000/08-11로
--       적었는데 실제로는 37.5665/08-12다). **총계는 그래도 맞아서 안 걸렸다** —
--       24줄·76건이 맞으니 그냥 넘어갈 수 있었다. 판정도 둘 다 🗑라서 uuid 대조로도
--       안 걸렸다. 걸린 이유는 좌표 무리의 **개수**(6 vs 5)가 안 맞아서다.
--       → 표를 검산할 때 **합계만 맞추면 안 된다.** 그리고 분류표의 `⚪️` 조건이
--         `round(lat,4)=37.5665 and round(lng,4)=126.9780` **그 자체**이므로,
--         분류표에서 ⚪️였던 계정은 좌표가 확정이다 — 그것으로 교차검증했다.
--
-- 🔵 지문 4개 중 `(37.5624, 126.9256)`은 이번 표에 안 나왔다(probe_ranking 계정이
--    이미 없다). **남는 지문은 아무것도 지우지 않으므로 해롭지 않다** — 지우지 않는다.
...
-- ─────────────────────────────────────────────────────────────
-- 3. 지운 뒤에 다시 센다 — **잠가 뒀다** (2절과 같은 이유다)
-- ─────────────────────────────────────────────────────────────
--
-- 🔴 **이 절도 `/* */`로 잠근다. 2026-08-12에 실제로 사고가 났기 때문이다.**
--
--    처음 판에서는 3절이 열려 있었다. 그래서 오너가 파일을 **통째로** 붙여넣었을 때
--    **SQL Editor가 마지막 문장만 보여줘서 3절 표만 떴고, 정작 봐야 할 1절
--    미리보기(`🗑 17줄`)가 조용히 가려졌다.** 아무 오류도 안 났고, 화면에는
--    그럴듯한 표가 하나 떠 있었다.
--
--    ⚠️ 나는 그 함정을 **이 파일 안에 경고로 적어 두고도** 1절과 3절을 둘 다
--       열어 놨다. 경고문은 그 일을 막지 못했다 — **막아야 할 것을 구조로 막지
--       않고 문장으로 부탁한 것**이 실수다. 이 저장소에서 같은 종류의 실수를
--       (금지어 목록 → 승인된 문구만 허용)에서 이미 한 번 뒤집었다.
--
--    → 이제 이 파일을 **통째로 붙여넣으면 1절 표가 나온다.** 그게 기본값이어야 한다.
--      1절은 아무것도 지우지 않으므로, 잘못 눌러도 사고가 아니다.
--
-- 🔵 3절은 **2절을 돌린 뒤에만** 의미가 있다. 지우기 전에 돌리면 "지운 뒤의 수"가
--    아니라 그냥 현재 수이고, 그걸 보고 **"다 됐다"고 읽게 된다**(실제로 그랬다).
...
-- ⚠️ **기대(2절을 돌린 뒤): 보존계정살아있나 = 1 · 실물QA_42건 = 42 ·
--    기록있는계정 24 → 7 · 남은기록 76 → 57.**
--    🔴 앞의 두 줄이 이 파일의 진짜 검사다. "몇 개 지웠다"는 성공을 증명하지 않는다 —
--    **지우면 안 되는 것이 남아 있는가**가 증명한다.
--
-- 🔵 **2026-08-12에 이 절만 열려서 나온 값을 기록해 둔다** (삭제 전 상태다):
--
--        남은익명계정 121 · 기록있는계정 24 · 남은기록 76 · 보존 1 · 실물QA 42
--
--    ⚠️ **`기록있는계정 24`가 "아직 아무것도 안 지워졌다"는 증거다** — 지웠다면
--       7이어야 한다. 그리고 `남은기록 76`은 오너 표 24줄의 기록수 합과 정확히
--       같다(⚪️6 + 🔴13 + 🟢57 = 76). 즉 **한 건도 안 지워졌다.**
--    ⚠️ `남은익명계정`이 110 → 121로 **늘어난 것도 정상이다.** 그 사이에 내가
--       댓글 삭제·Manual Linking을 실측하면서 익명 계정을 만들었다(기록은 안 만들었다).
--       🔴 이 수가 줄지 않는 것을 보고 "삭제가 실패했다"로 읽지 않는다 — 이 파일은
--       **기록이 있는 계정만** 지운다.
